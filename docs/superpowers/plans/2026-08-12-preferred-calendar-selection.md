...
The screen loads the calendar list whenever permission is granted — regardless of the checkbox — so the dropdown can legitimately be populated while disabled. This is also where a stale choice gets cleared, lazily, so the write path never blocks on a preference write.
...
Per `AGENTS.md` § Testing Conventions, all mock configuration lives in `createViewModel`. Replace it with:
...
Match the real `CalendarInfo` constructor order — use named arguments throughout.
...
Note `calendarSelected` persists `calendar.identity`, which is null for a calendar whose provider omitted part of the triple. Selecting such a calendar therefore stores "no choice" and the app keeps auto-picking — the honest outcome, since there is nothing stable to remember.
...
private fun CalendarSelectionDropdown(
    ...
    val selectedCalendar = calendars.resolvePreferred(preferredCalendar)
...
```kotlin
        SettingsSection(title = stringResource(R.string.settings_section_calendar)) {
            ...
            CalendarSelectionDropdown(
                ...
                preferredCalendar = uiState.preferredCalendar,
...
Match the real `CalendarInfo` constructor order and use named arguments. The preview needs
`import com.msmobile.visitas.util.identity` for `PREVIEW_MINISTRY_CALENDAR.identity`.
...
**Naming consistency, checked across tasks:** `resolvePreferred` (Tasks 2, 3, 6, 7), `orderedByAutoPickPreference` (Tasks 2, 3), `CalendarIdentity(accountType, accountName, ownerAccount)` and the `CalendarInfo.identity` accessor (Tasks 2, 3, 5, 6, 7, 8), `CalendarInfo(id, displayName, accountName, ownerAccount, accountType, isPrimary)` (every construction site must pass all six), `getAvailableCalendars` (Tasks 3, 6), `Preference.preferredCalendar` / `Preference.withPreferredCalendar` (Tasks 4, 5, 6), `UiEvent.CalendarSelected(calendar: CalendarInfo)` (Tasks 6, 7), `settings_calendar_selection_label` / `settings_calendar_selection_empty` (Task 7).
