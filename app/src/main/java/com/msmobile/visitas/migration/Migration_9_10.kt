...
val MIGRATION_9_10 = object : Migration(9, 10) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            ...
            |    PRIMARY KEY(`id`),
            |    FOREIGN KEY(`id`) REFERENCES `householder`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE
            |)
            """.trimMargin()
        )
        ...
        db.execSQL(
            ...
            |    PRIMARY KEY(`id`),
            |    FOREIGN KEY(`householderId`) REFERENCES `householder`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE
            |)
            """.trimMargin()
        )
